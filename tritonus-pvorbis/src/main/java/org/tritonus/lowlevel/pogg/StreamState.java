...
public class StreamState {
    ...
    public int init(int nSerialNo) {
        ...
        // TODO: necessary?
        Arrays.fill(m_abBodyData, (byte) 0);
        Arrays.fill(m_anLacingValues, 0);
        Arrays.fill(m_alGranuleValues, 0);
  ...
  /* If this is the initial header case, the first page must
     only include the initial header packet */
        ...
        if ((m_anLacingValues[0] & 0x100) == 0)
            m_abHeader[5] = (byte) (m_abHeader[5] | 0x01);
        /* first page flag? */
        if (!m_bBos)
            m_abHeader[5] = (byte) (m_abHeader[5] | 0x02);
        /* last page flag? */
        if (m_bEos && m_nLacingFill == vals)
            m_abHeader[5] = (byte) (m_abHeader[5] | 0x04);
        m_bBos = true;
